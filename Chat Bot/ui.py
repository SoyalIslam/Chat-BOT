...
if not st.session_state["logged_in"]:
    ...
    st.image(image, caption='My Chat Bot', use_container_width=True)
...
# Chat interface 
    ...
    # Side Logo
      ...
      st.sidebar.image(logo_image, caption='''Hi! Am here to Save Records''', use_container_width=True)

      for index, (sender, message) in enumerate(st.session_state.chat_history, start=1):
        st.sidebar.text_area(f"{sender} ({index}):", message, height=70 , max_chars=len(message), key=f"chat_history_{index}")
